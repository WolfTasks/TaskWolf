...
3. **`useLanguageSync()`** (`frontend/src/hooks/useLanguageSync.ts`) is called once from `AppLayout`. After `/me` loads, if the server has a stored `language` different from the current i18next language, it applies it — but only once per session (guarded by a ref) and only if localStorage hadn't already set a language on this device.
